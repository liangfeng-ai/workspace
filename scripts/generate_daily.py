...
资源可访问性要求：
- 所有阅读、听力、视频、文字稿、练习所依赖的外部材料都必须可以免费获取完整内容。
- 禁止使用需要订阅、会员、付费、登录、注册、App 内打开、校园/机构权限或试读后截断的材料。
- 禁止使用明显带 paywall 的来源或页面；如果页面提示 réservé aux abonnés、article réservé、abonnement、subscriber-only、premium、paywall、login required 等，必须换来源。
- 优先选择 RFI、France 24、TV5MONDE、Radio France、France Culture、Arte、Vie publique、INSEE、CNRS、The Conversation France、Le Monde diplomatique 免费文章、官方机构网站、大学/研究机构公开页面等可直接访问完整内容的来源。
- 给出的链接必须是具体文章、具体音频、具体视频或具体 transcript 页面，不能是官网首页、栏目页、搜索页、标签页或集合页。
- 在“今日阅读材料”中增加一行“免费可访问性检查”，明确说明该材料为何无需订阅、无需登录即可完整学习。
...
- 给出免费可访问性检查：说明该页面可以直接打开全文，或直接提供完整音频/视频/文字稿；如果无法确认免费完整访问，请不要使用该材料。
...
def validate_french_accessibility(content: str) -> None:
    blocked_terms = (
        "réservé aux abonnés",
        "reserve aux abonnes",
        "article réservé",
    ...
    matches = [term for term in blocked_terms if term in lowered]
    if matches:
        raise ValueError(
            "French study output appears to reference restricted material: "
            + ", ".join(matches)
        )
...
def main() -> None:
    ...
    if args.task == "french":
        validate_french_accessibility(content)
